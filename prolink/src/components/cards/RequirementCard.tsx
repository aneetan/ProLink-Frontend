...
import { Edit, Trash2, DollarSign, Calendar, MapPin, Tag, AlertCircle, Users, Clock, TrendingUp, FileText, Briefcase, ExternalLink, ChevronRight, CheckCircle, XCircle } from 'lucide-react';
...
const RequirementCard: React.FC<RequirementCardProps> = ({
  ...
  const [isCheckingBid, setIsCheckingBid] = useState(true);

  // Check if company has already submitted a bid for this requirement
  useEffect(() => {
    if (isCompany && token && requirement?.id) {
      checkBidStatus();
    } else {
      setIsCheckingBid(false);
    }
  }, [isCompany, token, requirement?.id]);

  // Function to check bid status
  const checkBidStatus = async () => {
    try {
      ...
      const response = await requestBidService.checkBidStatus(requirement.id, companyId);
      
      if (response?.hasSubmitted) {
        setHasSubmittedBid(true);
        setBidStatus(response.bid);
      } else {
  ...
  // Mutation for submitting bid
  const mutation = useMutation({
    mutationFn: requestBidService.sendQuote,
    onSuccess: () => {
      showSuccessToast(`Quote submitted for ${requirement.title}`);
      // Update bid status after successful submission
      setHasSubmittedBid(true);
      // You might want to fetch the actual bid data here
    },
    onError: (e) => {
      console.log(e);
  ...
  // Handle decline (if you want to implement this)
  const handleDecline = async () => {
    // Implement decline logic if needed
    console.log('Declined requirement:', requirement.id);
  };
  ...
  // Render button based on bid status
  const renderCompanyButton = () => {
    if (isCheckingBid) {
      return (
        <button
          disabled
          className="group w-full sm:w-auto px-6 py-3 bg-gray-300 text-gray-500 rounded-xl font-semibold cursor-not-allowed flex items-center justify-center gap-3"
        >
    ...
    if (hasSubmittedBid) {
      const isPending = bidStatus?.status === 'PENDING';
      const isAccepted = bidStatus?.status === 'ACCEPTED';
      const isRejected = bidStatus?.status === 'REJECTED';

      return (
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <button
            disabled
            className={`group w-full sm:w-auto px-6 py-3 rounded-xl font-semibold flex items-center justify-center gap-3 ${
              isPending 
          ...
          >
            {isPending ? (
              <>
                <Clock size={18} />
                <span>Submitted</span>
                <span className="px-2 py-1 text-xs font-bold bg-amber-200 rounded-full">
                  Pending
                </span>
              </>
            ) : isAccepted ? (
              <>
                <CheckCircle size={18} />
                <span>Accepted</span>
                <span className="px-2 py-1 text-xs font-bold bg-emerald-200 rounded-full">
                  Accepted
                </span>
              </>
            ) : (
              <>
                <XCircle size={18} />
                <span>Rejected</span>
                <span className="px-2 py-1 text-xs font-bold bg-rose-200 rounded-full">
                  Rejected
                </span>
              </>
            )}
          ...
          {bidStatus && (
            <div className="text-sm text-gray-600 text-center sm:text-left">
              <div className="font-medium">Your Bid: {formatCurrency(bidStatus.amount)}</div>
              <div className="text-xs">Submitted: {new Date(bidStatus.createdAt).toLocaleDateString()}</div>
            </div>
          )}
        </div>
      );
    ...
    // If no bid submitted, show submit button
    return (
      <div className='flex justify-end items-center gap-4'>
        ...
        <button
          ...
          className="group w-full sm:w-auto px-6 py-3 bg-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-300 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-3"
        >
          Decline
        </button>
      </div>
